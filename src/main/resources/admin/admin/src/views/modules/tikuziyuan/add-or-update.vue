...
export default {
  ...
  methods: {
	...
	this.ruleForm.ziyuanneirong = this.ruleForm.ziyuanneirong.replace(reg,'../../../springbootx1786/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tikuziyuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tikuziyuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
